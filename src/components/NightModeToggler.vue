<template>
  <input
    type="checkbox"
    name="checkbox"
    :class="['switch', {'switch--active': modelValue}]"
    :checked="modelValue"
...
.switch:after {
  content: '';
  width: calc(var(--width) / 2.83);
  height: calc(var(--width) / 2.83);
  border-radius: 50%;
  background-color: #fff;
  position: absolute;
  left: calc(var(--width) / 65) ;
  top: calc(var(--height) / 25);
  transform: translateX(0px);
  animation: off .5s forwards ease-in-out;
  box-shadow: inset 5px -5px 4px rgba(53, 53, 53, 0.3);
}

@keyframes off {
  0% {
    transform: translateX(calc(var(--width) / 1.62));
    width: calc(var(--width) / 2.83);
  }
  50% {
    width: calc(var(--width) / 1.73);  
    border-radius: 9999px;   
  }
  100% {
    transform: translateX(0px); 
...
.switch:checked:after {
  animation: on .5s forwards ease-in-out;
  box-shadow: inset -5px -5px 4px rgba(53, 53, 53, 0.3);
}
...
.switch:checked:before {
  content: '';
  width: calc(var(--width) / 8.66);
  height: calc(var(--width) / 8.66);
  border-radius: 50%;
  position: absolute;
  left: calc(var(--width) / 8.66);
  top: calc(var(--height) / 10);
  transform-origin: calc(var(--height) / 0.94) calc(var(--width) / 13);
